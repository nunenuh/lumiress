...
class WeightDownloader:
    ...
    def download_weights(self):
        ...
        self.tqdm_instance = tqdm(
            unit="B", unit_scale=True, miniters=1, desc="Downloading", leave=True
        )
        urllib.request.urlretrieve(
            self.weight_url, self.weight_path, reporthook=self._reporthook
        )
        ...
        while retries > 0:
            ...
            if self.verbose:
                logging.info(
                    f"File checksum does not match. Removing file: {self.weight_path}"
                )
        ...
        if self.verbose:
            ...
            logging.info(
                "file SHA256SUM: {}".format(self._calculate_sha256(self.weight_path))
            )
